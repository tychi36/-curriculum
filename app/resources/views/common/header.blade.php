                ...
                <h1 class="fs-1 title"><a href="{{ route('posts.index') }}">Poweeer<i class="fas fa-solid fa-dumbbell fa-2xs titile_logo"></i></a></h1>
                    <div class="">
                        @can('admin')
                            ...
                            @if(Auth::check())
                            <div class="ml-2">
                                <form class="d-flex mb-3" action="{{ route('search_host') }}" method="GET">
                                    @csrf
                        ...
                        @elsecan('user')
                            <div class="" id="navbarSupportedContent">
                                ...
                                @if(!Route::is('posts.create','users.show','users.edit','posts.show','weight_mgmts.index','weight_mgmts.create','weight_mgmts.show','weight_mgmts.edit'))
                                <form class="d-flex mb-3" action="{{ route('search') }}" method="GET">
                                    @csrf
                                    <input class="form-control me-2 width_reset" placeholder="Search" aria-label="Search" type="search" name="search" value="@if (isset($search)) {{ $search }} @endif">
                                    <button class="btn btn-outline-success" type="submit">Search</button>
                                </form>
